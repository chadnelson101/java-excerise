...
let input = document.querySelector['data-firstMultiply']

// 5.2. Attach the second input to a variable
let input1 = document.querySelector['data-secondMultiply']
...
let x = document.querySelector('data-first')
console.log();
...
function multiply (){
    let x =innerText = eval(input.value * input1.value)

    let answer = (`${input} * ${input1}`)
    answer.innerText = x
}
// 5.6. Add an event listener to the button that runs the function that tyou created
let z = document.addEventListener('click',y)
